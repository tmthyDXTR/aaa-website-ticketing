...
export const createOrderSQL = async (jsonResponse=null, cart = null, isPaypalOrder=true) => {
    ...
    if (isPaypalOrder)
    {
      ...
      var sql = "INSERT INTO aaa_orders (paypal_order_id, order_status, order_create_time, order_update_time) VALUES ('"+vkId+"', 'CREATED', '"+new Date().toISOString().slice(0, 19).replace('T', ' ')+"', '"+new Date().toISOString().slice(0, 19).replace('T', ' ')+"')";
    }
